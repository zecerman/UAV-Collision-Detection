...
public class LiDARLogger : MonoBehaviour
{
    ...
    void DoScan(bool updateCsv)
    {
        ...
        // 3) Fill latestRow (header + motors + distances)
        int p = 0;
        latestRow[p++] = t;
        ...
        if (updateCsv && _writer != null)
        {
            ...
            // motors
            for (int i3 = 0; i3 < motorCount; i3++)
                row.Add(latestRow[r++].ToString("F3", inv));
            // distances
